		 	...
		 	<div class="col-md-8">
		 		<table class="table table-hover">
					...
					<tbody>
						...
						include("db.php");
						$result = mysqli_query($conn, "SELECT bet.MatchID, bet.CustomerID, bet.whichTeam, bet.money, matches.team1, matches.team2, matches.point1, matches.time, matches.point2, matches.isEnd FROM bet INNER JOIN matches ON bet.MatchID = matches.MatchID WHERE bet.CustomerID = '$id' && matches.isEnd = 1");
				
						while ($bet = mysqli_fetch_assoc($result)) {
								if ($bet['whichTeam'] == 1) {
									if ($bet['point1'] > $bet['point2']) echo "<tr class = 'success'>";
									if ($bet['point1'] < $bet['point2']) echo "<tr class = 'danger'>";
								}
								if ($bet['whichTeam'] == 2) {
									if ($bet['point1'] < $bet['point2']) echo "<tr class = 'success'>";
									if ($bet['point1'] > $bet['point2']) echo "<tr class = 'danger'>";
